...
<body>
    ...
    <div id="lista">
        <ul>
            ...
            <li><a href="./osobowe.php" id="us_visi">Samochody osobowe</a></li>
    ...
    <div id="main_reszt"> 
        ...
        $query=mysqli_query($connect, "SELECT `ID_pojazdu`,`Rejestracja`, `Model`, `Marka`, `Opis`, `Cena`, `Typ`,`Ilosc miejsc`, `Zdjecie` FROM `pojazdy` where `Typ`='Osobowy'");

        if(isset($_POST['submit']))
        {
            $sort=$_POST['sortowanie'];
            $query=mysqli_query($connect, "SELECT `ID_pojazdu`,`Rejestracja`, `Model`, `Marka`, `Opis`, `Cena`, `Typ`,`Ilosc miejsc`, `Zdjecie` FROM `pojazdy` where `Typ`='Osobowy' ORDER BY `Cena` $sort");             
        }
        ...
        {   
        echo "<div class='pojazd pojazd$j'><div class='dane'><img src='./Photos/Osobowe/$tablica[Zdjecie]'><p class='danep'>".$tablica['Marka']." ".$tablica['Model']."<br>";
        echo "</p>";
