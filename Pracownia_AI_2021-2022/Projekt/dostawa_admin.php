...
<body>
    ...
    <div id="lista">
        <ul>
            ...
            <li><a href="./bus.php">Busy</a></li>
            <li><a href="./dostawa.php" id="us_visi">Samochody dostawcze</a></li>
    ...
    <div id="main_reszt"> 
        ...
        $query=mysqli_query($connect, "SELECT `ID_pojazdu`,`Rejestracja`, `Model`, `Marka`, `Opis`, `Cena`, `Typ`,`Ilosc miejsc`, `Zdjecie` FROM `pojazdy` where `Typ`='Dostawczy'");

        if(isset($_POST['submit']))
        {
            $sort=$_POST['sortowanie'];
            $query=mysqli_query($connect, "SELECT `ID_pojazdu`,`Rejestracja`, `Model`, `Marka`, `Opis`, `Cena`, `Typ`,`Ilosc miejsc`, `Zdjecie` FROM `pojazdy` where `Typ`='Dostawczy' ORDER BY `Cena` $sort");             
        }
