...
<body>
    ...
    <div id="lista">
        <ul>
            ...
            <li><a href="./dostawa_user.php" id="us_visi">Samochody dostawcze</a></li>
    ...
    <div id="main_reszt"> 
        ...
        $query=mysqli_query($connect, "SELECT `Rejestracja`, `Model`, `Marka`, `Opis`, `Cena`, `Typ`,`Ilosc miejsc`, `Zdjecie` FROM `pojazdy` where `Typ`='Dostawczy'");

        if(isset($_POST['submit']))
        {
            $sort=$_POST['sortowanie'];
            $query=mysqli_query($connect, "SELECT `Rejestracja`, `Model`, `Marka`, `Opis`, `Cena`, `Typ`,`Ilosc miejsc`, `Zdjecie` FROM `pojazdy` where `Typ`='Dostawczy' ORDER BY `Cena` $sort");             
        }
        ...
        {   
        echo "<div class='pojazd pojazd$j'><div class='dane'><img src='./Photos/Dostawcze/$tablica[Zdjecie]'><p class='danep'>".$tablica['Marka']." ".$tablica['Model']."<br>";
        echo "</p>";
        echo "<p class='opis'>".$tablica['Opis']."</p><p class='cena'>".$tablica['Cena']." zł/dzień </p></div>";
        echo "<div class='przycisk'><form action='rezerwacjaD.php' method='POST'><input type='submit' value='Rezerwuj pojazd' name='submit$i'>";
        echo "<input type='text' name='marka' value='$tablica[Marka]' hidden><input type='text' name='rejestracja' value='$tablica[Rejestracja]' hidden><input type='text' name='model' value='$tablica[Model]' hidden>";
        echo "</form></div></div>";
        $i++;
